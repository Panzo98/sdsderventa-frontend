...
export default function NewPosition() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (title === "")
      return alert("Поље за назив позиције не може бити празно!");
    try {
      ...
      alert("Успјешно сте креирали позицију!");
      setTitle("");
      dispatch({ type: "UPDATE_POSITIONS", payload: response.data.position });
    } catch (error) {
      alert("Грешка приликом креирања позиције!");
    }
  };
  return (
    <div className="w-full py-4 px-5 flex flex-col   justify-center rounded-lg shadow-xl border-solid border-gray-100 border-[1px] mb-4">
      ...
      >
        <input
          placeholder="Унесите назив позиције"
          type="text"
