...
export default function TableRow({ data, newId }) {
  const dispatch = useDispatch();
  const handleDelete = async () => {
    try {
      let response = await axios.delete(
        `${process.env.REACT_APP_APP_URL}/users/delete-member/${data.id}`,
  ...
  return (
    <div className=" border-b-[1px] flex w-full text-[6px] xl:text-sm">
      ...
      <span className=" items-center flex w-[20%] pl-2 border-r-[1px] border-t-[1px] py-1 xl:py-3">
        {data.firstName}
      </span>
      <span className=" items-center flex w-[20%] pl-2 border-r-[1px] border-t-[1px] py-1 xl:py-3">
        {data.lastName}
      </span>
      <span className=" items-center flex w-[15%] justify-center border-r-[1px] border-t-[1px] py-1 xl:py-3">
        {data.odborTitle}
      </span>
      <span className=" items-center flex w-[15%] justify-center border-r-[1px] border-t-[1px] py-1 xl:py-3">
        {data.positionTitle}
      ...
      <span
        className=" items-center flex w-[15%] border-r-[1px] border-t-[1px] xl:text-xs py-1 xl:py-3 justify-center cursor-pointer"
        onClick={() => (window.location = `tel:${data.phoneNumber}`)}
      >
        {data.phoneNumber}
      </span>
